...
@php
    ...
    // Right Side Illustration settings
    $rightSideImageEnabled = AlumniSetting::getValue('right_side_image_enabled', 'false') === 'true';
    $rightSideImageUrl = AlumniSetting::getValue('right_side_image_url', '');
    $rightSideImageAlt = AlumniSetting::getValue('right_side_image_alt', 'Ilustrasi Alumni Sistem Informasi');
    $rightSideFallbackToIllustration = AlumniSetting::getValue('right_side_fallback_to_illustration', 'false') === 'true';
    $rightSideEmptyMessage = AlumniSetting::getValue('right_side_empty_message', 'Silakan tambahkan gambar melalui pengaturan alumni');
    $rightSideShowEmptyState = AlumniSetting::getValue('right_side_show_empty_state', 'true') === 'true';
@endphp
    ...
    <div class="container" style="position: relative; z-index: 2;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 60px; align-items: center;" data-aos="fade-up">
            ...
            <div style="position: relative; text-align: center;" data-aos="fade-left" data-aos-delay="200">
                @if($rightSideImageEnabled && !empty($rightSideImageUrl))
                    <!-- Dynamic Image from Database -->
                    <div style="position: relative; display: inline-block;">
                        <img src="{{ $rightSideImageUrl }}"
                             alt="{{ $rightSideImageAlt }}"
                             style="max-width: 100%; height: auto; border-radius: 20px; box-shadow: 0 20px 40px rgba(0,0,0,0.1);"
                             loading="lazy">
                    </div>
                @elseif($rightSideFallbackToIllustration)
                    <!-- Fallback to CSS Illustration -->
                    <div style="position: relative; display: inline-block;">
                ...
                @elseif($rightSideShowEmptyState)
                    <!-- Empty State Message -->
                    <div style="position: relative; display: inline-block; min-height: 400px; width: 100%;">
                        <div style="display: flex; justify-content: center; align-items: center; height: 400px; flex-direction: column; gap: 20px;">
                            <div style="width: 80px; height: 80px; background: linear-gradient(135deg, #f0f4ff 0%, #e8eaf6 100%); border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
                                <i class="fas fa-image" style="font-size: 32px; color: #cbd5e1;"></i>
                            </div>
                            <div style="text-align: center; color: #64748b; font-size: 16px; max-width: 300px;">
                                <p style="margin: 0; font-weight: 600; color: #475569;">Gambar belum tersedia</p>
                                <p style="margin: 8px 0 0 0; font-size: 14px; line-height: 1.6;">{{ $rightSideEmptyMessage }}</p>
                            </div>
                        </div>
                    </div>
                @endif

                @if($rightSideImageEnabled || $rightSideFallbackToIllustration)
...
@php
    ...
    try {
        ...
        $academicInfoEnabled = HomeSetting::where('key', 'academic_info_enabled')->value('value') === 'true';
        $academicInfoLayoutStyle = HomeSetting::where('key', 'academic_info_layout_style')->value('value') ?? 'cards';
        $academicInfoSectionTitle = HomeSetting::where('key', 'academic_info_section_title')->value('value') ?? 'INFORMASI AKADEMIK';
        $academicInfoSectionSubtitle = HomeSetting::where('key', 'academic_info_section_subtitle')->value('value') ?? 'Informasi terkini seputar penerimaan mahasiswa dan kegiatan akademik';
        $academicInfoSectionIcon = HomeSetting::where('key', 'academic_info_section_icon')->value('value') ?? 'fa-graduation-cap';

        // PMB Info
        $pmbInfoEnabled = HomeSetting::where('key', 'pmb_info_enabled')->value('value') === 'true';
        $pmbInfoTitle = HomeSetting::where('key', 'pmb_info_title')->value('value') ?? 'PENERIMAAN MAHASISWA BARU {{year}}';
        $pmbInfoTitle = str_replace('{{year}}', date('Y'), $pmbInfoTitle);
        $pmbInfoDescription = HomeSetting::where('key', 'pmb_info_description')->value('value') ?? 'Pendaftaran mahasiswa baru Program Studi Sistem Informasi telah dibuka. Segera daftarkan diri Anda!';
        $pmbInfoLink = HomeSetting::where('key', 'pmb_info_link')->value('value') ?? '#';
        $pmbInfoButtonText = HomeSetting::where('key', 'pmb_info_button_text')->value('value') ?? 'Daftar Sekarang';
        $pmbInfoStatus = HomeSetting::where('key', 'pmb_info_status')->value('value') ?? 'active';
        $pmbInfoDeadline = HomeSetting::where('key', 'pmb_info_deadline')->value('value') ?? '';
        $pmbInfoQuota = HomeSetting::where('key', 'pmb_info_quota')->value('value') ?? '';

        // Alert System
        $alertEnabled = HomeSetting::where('key', 'alert_enabled')->value('value') === 'true';
        $alertType = HomeSetting::where('key', 'alert_type')->value('value') ?? 'info';
        $alertTitle = HomeSetting::where('key', 'alert_title')->value('value') ?? 'Pengumuman';
        $alertMessage = HomeSetting::where('key', 'alert_message')->value('value') ?? '';
        $alertLink = HomeSetting::where('key', 'alert_link')->value('value') ?? '#';
        $alertButtonText = HomeSetting::where('key', 'alert_button_text')->value('value') ?? 'Lihat Detail';
        $alertClosable = HomeSetting::where('key', 'alert_closable')->value('value') === 'true';

        // Info Cards
        $card1Enabled = HomeSetting::where('key', 'card_1_enabled')->value('value') === 'true';
        $card1Title = HomeSetting::where('key', 'card_1_title')->value('value') ?? '';
        $card1Description = HomeSetting::where('key', 'card_1_description')->value('value') ?? '';
        $card1Icon = HomeSetting::where('key', 'card_1_icon')->value('value') ?? 'fa-info-circle';
        $card1Link = HomeSetting::where('key', 'card_1_link')->value('value') ?? '#';
        $card1Color = HomeSetting::where('key', 'card_1_color')->value('value') ?? '#10b981';

        $card2Enabled = HomeSetting::where('key', 'card_2_enabled')->value('value') === 'true';
        $card2Title = HomeSetting::where('key', 'card_2_title')->value('value') ?? '';
        $card2Description = HomeSetting::where('key', 'card_2_description')->value('value') ?? '';
        $card2Icon = HomeSetting::where('key', 'card_2_icon')->value('value') ?? 'fa-info-circle';
        $card2Link = HomeSetting::where('key', 'card_2_link')->value('value') ?? '#';
        $card2Color = HomeSetting::where('key', 'card_2_color')->value('value') ?? '#f59e0b';

        $card3Enabled = HomeSetting::where('key', 'card_3_enabled')->value('value') === 'true';
        $card3Title = HomeSetting::where('key', 'card_3_title')->value('value') ?? '';
        $card3Description = HomeSetting::where('key', 'card_3_description')->value('value') ?? '';
        $card3Icon = HomeSetting::where('key', 'card_3_icon')->value('value') ?? 'fa-info-circle';
        $card3Link = HomeSetting::where('key', 'card_3_link')->value('value') ?? '#';
        $card3Color = HomeSetting::where('key', 'card_3_color')->value('value') ?? '#3b82f6';

        // Theme Colors
        $academicInfoBgColor = HomeSetting::where('key', 'academic_info_bg_color')->value('value') ?? '#f8fafc';
        $academicInfoAccentColor = HomeSetting::where('key', 'academic_info_accent_color')->value('value') ?? '#1a246a';
        $academicInfoTextColor = HomeSetting::where('key', 'academic_info_text_color')->value('value') ?? '#1e293b';
        $academicInfoSecondaryText = HomeSetting::where('key', 'academic_info_secondary_text')->value('value') ?? '#64748b';

    } catch (\Exception $e) {
        $academicInfoEnabled = false;
        $academicInfoLayoutStyle = 'current';
        $academicInfoBadgeText = 'BANNER INFORMASI';
        $academicInfoBadgeIcon = 'fa-bullhorn';
        $academicInfoTitle = 'Informasi Penerimaan Mahasiswa Baru ' . date('Y');
        $academicInfoSubtitle = 'Program Studi Sistem Informasi Universitas Bengkulu';
        $academicInfoButtonText = 'Info Selengkapnya';
        $academicInfoButtonLink = '/contact';
        $academicInfoButtonIcon = 'fa-arrow-right';
        $academicInfoBgColor = '#f8fafc';
        $academicInfoBorderColor = '#e5e7eb';
        $academicInfoBadgeBg = '#f1f5f9';
        $academicInfoBadgeDotColor = '#3b82f6';
        $academicInfoTitleColor = '#1e293b';
        $academicInfoSubtitleColor = '#64748b';
        $academicInfoButtonBg = '#2563eb';
        $academicInfoButtonTextColor = '#ffffff';
        $academicInfoButtonBorderColor = '#2563eb';

        // Alert System Defaults
        $alertEnabled = false;
        $alertType = 'info';
        $alertTitle = 'Pengumuman';
        $alertMessage = '';
        $alertLink = '#';
        $alertButtonText = 'Lihat Detail';
        $alertClosable = true;

        // Cards Defaults
        $card1Enabled = $card2Enabled = $card3Enabled = false;
        $card1Title = $card2Title = $card3Title = '';
        $card1Description = $card2Description = $card3Description = '';
        $card1Icon = $card2Icon = $card3Icon = 'fa-info-circle';
        $card1Link = $card2Link = $card3Link = '#';
        $card1Color = '#10b981';
        $card2Color = '#f59e0b';
        $card3Color = '#3b82f6';
    }
...
<!-- Informasi Akademik Section - Modern -->
@if($academicInfoEnabled)
<section style="padding: 80px 0; background: {{ $academicInfoBgColor }};">
    <div class="container" data-aos="fade-up">

        <!-- Section Header -->
        <div style="text-align: center; margin-bottom: 60px;">
            <div style="display: inline-flex; align-items: center; gap: 12px; padding: 8px 20px; background: linear-gradient(135deg, {{ $academicInfoAccentColor }}, {{ $academicInfoAccentColor }}CC); border-radius: 50px; margin-bottom: 24px; box-shadow: 0 8px 25px rgba(26, 36, 106, 0.15);">
                <i class="fas {{ $academicInfoSectionIcon }}" style="color: #fff; font-size: 16px;"></i>
                <span style="font-size: 14px; font-weight: 700; color: #fff; letter-spacing: 1px; text-transform: uppercase;">{{ $academicInfoSectionTitle }}</span>
            </div>
            <h2 style="font-size: 42px; font-weight: 800; color: {{ $academicInfoTextColor }}; margin: 0 0 16px 0; line-height: 1.2;">
                Informasi Akademik <span style="background: linear-gradient(135deg, {{ $academicInfoAccentColor }}, #3b82f6); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">Terkini</span>
            </h2>
            <p style="font-size: 18px; color: {{ $academicInfoSecondaryText }}; max-width: 700px; margin: 0 auto; line-height: 1.6;">
                {{ $academicInfoSectionSubtitle }}
            </p>
        </div>

        @if($academicInfoLayoutStyle === 'cards')
        <!-- Layout Cards Grid -->
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 24px;">

            <!-- Alert/Pengumuman Priority -->
            @if($alertEnabled && !empty($alertMessage))
                <div style="grid-column: 1 / -1;" data-aos="fade-up" data-aos-delay="100">
                    <div style="background: white; border-radius: 16px; padding: 24px; border: 1px solid #e5e7eb; box-shadow: 0 4px 6px rgba(0,0,0,0.07); position: relative;">
                        @if($alertClosable)
                        <button onclick="this.parentElement.style.display='none'" style="position: absolute; top: 16px; right: 16px; background: none; border: none; color: #64748b; cursor: pointer; font-size: 20px; line-height: 1;">×</button>
                        @endif

                        <!-- Alert Icon & Type Badge -->
                        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
                            <div style="width: 48px; height: 48px; background:
                                @if($alertType === 'warning') linear-gradient(135deg, #f59e0b, #d97706)
                                @elseif($alertType === 'success') linear-gradient(135deg, #10b981, #059669)
                                @elseif($alertType === 'error') linear-gradient(135deg, #ef4444, #dc2626)
                                @else linear-gradient(135deg, #3b82f6, #2563eb)
                                @endif
                                ; border-radius: 12px; display: flex; align-items: center; justify-content: center;">
                                <i class="fas
                                    @if($alertType === 'warning') fa-exclamation-triangle
                                    @elseif($alertType === 'success') fa-check-circle
                                    @elseif($alertType === 'error') fa-exclamation-circle
                                    @else fa-info-circle
                                    @endif
                                    " style="color: #fff; font-size: 24px;"></i>
                            </div>
                            <div>
                                <div style="display: inline-flex; align-items: center; gap: 6px; padding: 4px 12px; background:
                                    @if($alertType === 'warning') #fef3c7
                                    @elseif($alertType === 'success') #d1fae5
                                    @elseif($alertType === 'error') #fee2e2
                                    @else #dbeafe
                                    @endif
                                    ; border-radius: 20px; margin-bottom: 8px;">
                                    <span style="font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;
                                        @if($alertType === 'warning') color: #92400e
                                        @elseif($alertType === 'success') color: #065f46
                                        @elseif($alertType === 'error') color: #991b1b
                                        @else color: #1e40af
                                        @endif
                                    ;">{{ $alertTitle }}</span>
                                </div>
                                <h3 style="font-size: 20px; font-weight: 700; color: {{ $academicInfoTextColor }}; margin: 0 0 8px 0;">{{ $alertTitle }}</h3>
                            </div>
                        </div>
                        <p style="font-size: 16px; color: {{ $academicInfoSecondaryText }}; line-height: 1.6; margin: 0 0 20px 0;">{{ $alertMessage }}</p>
                        @if(!empty($alertLink) && $alertLink !== '#')
                        <a href="{{ $alertLink }}" style="display: inline-flex; align-items: center; gap: 8px; padding: 10px 20px; background:
                            @if($alertType === 'warning') #f59e0b
                            @elseif($alertType === 'success') #10b981
                            @elseif($alertType === 'error') #ef4444
                            @else #3b82f6
                            @endif
                            ; color: #fff; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 14px; transition: all 0.2s;"
            ...
            <!-- PMB Featured Card -->
            @if($pmbInfoEnabled)
            <div style="grid-column: span {{ $alertEnabled ? 1 : 2 }};" data-aos="fade-up" data-aos-delay="{{ $alertEnabled ? 200 : '100' }}">
            </a>
        </div>
        @endif
    </div>
    ...
    <div class="container" style="position: relative; z-index: 2;">
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 40px;">
            ...
            <div>
                ...
                <p style="color: {{ $footerColors['text_color'] }}; font-size: 13px; line-height: 1.7; margin: 0; max-width: 280px;">
            ...
            <div>
                ...
                <ul style="list-style: none; padding: 0; margin: 15px 0 0 0;">
                    ...
                    @else
                        ...
                        <li style="margin-bottom: 12px;">
                            <a href="#" style="color: {{ $footerColors['text_color'] }}; text-decoration: none; transition: all 0.3s; font-size: 14px; display: flex; align-items: center;">
                                <span style="width: 8px; height: 8px; background: linear-gradient(135deg, {{ $footerColors['bg_gradient_mid'] }}, {{ $footerColors['bg_gradient_end'] }}); border-radius: 50%; margin-right: 12px;"></span>
                                About Us
                            </a>
                        </li>
        ...
        <div style="display: flex; justify-content: center; gap: 15px; margin-top: 50px; padding-top: 40px; border-top: 1px solid rgba(255,255,255,0.2);">
            <a href="{{ $footerSocial['facebook'] }}" style="width: 40px; height: 40px; background: rgba(255,255,255,0.1); backdrop-filter: blur(10px); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: {{ $footerColors['heading_color'] }}; text-decoration: none; transition: all 0.3s; border: 1px solid rgba(255,255,255,0.2);">
                <i class="{{ $icons['facebook'] }}" style="font-size: 16px;"></i>
            </a>
            <a href="{{ $footerSocial['instagram'] }}" style="width: 40px; height: 40px; background: rgba(255,255,255,0.1); backdrop-filter: blur(10px); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: {{ $footerColors['heading_color'] }}; text-decoration: none; transition: all 0.3s; border: 1px solid rgba(255,255,255,0.2);">
                <i class="{{ $icons['instagram'] }}" style="font-size: 16px;"></i>
            </a>
            <a href="{{ $footerSocial['linkedin'] }}" style="width: 40px; height: 40px; background: rgba(255,255,255,0.1); backdrop-filter: blur(10px); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: {{ $footerColors['heading_color'] }}; text-decoration: none; transition: all 0.3s; border: 1px solid rgba(255,255,255,0.2);">
                <i class="{{ $icons['linkedin_in'] }}" style="font-size: 16px;"></i>
            </a>
            <a href="{{ $footerSocial['youtube'] }}" style="width: 40px; height: 40px; background: rgba(255,255,255,0.1); backdrop-filter: blur(10px); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: {{ $footerColors['heading_color'] }}; text-decoration: none; transition: all 0.3s; border: 1px solid rgba(255,255,255,0.2);">
                <i class="{{ $icons['youtube'] }}" style="font-size: 16px;"></i>
            </a>
        </div>
    </div>
</footer>
