                ...
                <div class="dosen-slides" style="display: flex; transition: transform 0.6s ease;">
                    ...
                    <div class="dosen-slide" style="min-width: 100%; display: grid; grid-template-columns: repeat(4, 1fr); gap: 25px; padding: 0 20px;">
                        ...
                        <div class="dosen-card-clean" style="background: #fff; border-radius: 16px; overflow: hidden; border: 1px solid #e5e7eb; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);">
                            <!-- Photo Area -->
                            <div style="position: relative; height: 280px; background: linear-gradient(135deg, #1a246a, #151945); overflow: hidden;">
                                <!-- Special badge for Kaprodi -->
                                @if(isset($dosenData[0]['badge_color']) && $dosenData[0]['badge_color'])
                                <div style="position: absolute; top: 16px; left: 16px; background: {{ $dosenData[0]['badge_color'] }}; color: #78350f; padding: 6px 12px; border-radius: 8px; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; z-index: 2;">
                                    Kaprodi
                                </div>
                                @endif

                                @if(isset($dosenData[0]['photo']) && $dosenData[0]['photo'])
                                    <!-- Photo -->
                                    <img src="{{ asset('storage/' . $dosenData[0]['photo']) }}" alt="{{ $dosenData[0]['name'] }}" style="width: 100%; height: 100%; object-fit: cover;">
                                @else
                                    ...
                                    <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center;">
                                        <div style="width: 80px; height: 80px; background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 12px;">
                                            <i class="fas {{ $dosenData[0]['icon'] ?? 'fa-user-tie' }}" style="font-size: 32px; color: #fff;"></i>
                                        </div>
                                    </div>
                                @endif
                            </div>
