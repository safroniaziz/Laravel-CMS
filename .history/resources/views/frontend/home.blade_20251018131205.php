        ...
        <div class="hero-slider" style="position: relative;">
            <div class="hero-slides" style="display: flex; transition: transform 0.8s ease-in-out;">
                @foreach($sliders as $index => $slider)
                    <div class="hero-slide" style="min-width: 100%; background: linear-gradient(135deg, #0f172a 0%, #1e3a8a 50%, #3b82f6 100%); padding: 80px 0;">
                        <div class="container">
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 60px; align-items: center; {{ $slider->image_position === 'left' ? 'direction: rtl;' : '' }}">
                                <!-- Content Side -->
                                <div style="color: #fff; {{ $slider->image_position === 'left' ? 'direction: ltr;' : '' }}">
                                    @if($slider->subtitle)
                                    ...
                                    <h1 style="font-size: 48px; font-weight: 900; margin: 0 0 20px 0; line-height: 1.2;">
                                        {{ $slider->title }}
                                    </h1>
                                    
                                    @if($slider->description)
                                        <p style="font-size: 17px; line-height: 1.8; margin-bottom: 30px; color: rgba(255,255,255,0.95);">
                                            {{ $slider->description }}
                                        </p>
                                    @endif
                                ...
                                <!-- Image Side -->
                                <div style="{{ $slider->image_position === 'left' ? 'direction: ltr;' : '' }}">
                                    @if($slider->image)
                                        <div style="position: relative; border-radius: 20px; overflow: hidden; box-shadow: 0 20px 60px rgba(0,0,0,0.4);">
                                            <img src="{{ $slider->image }}" alt="{{ $slider->title }}" style="width: 100%; height: auto; display: block;">
                                            <!-- Decorative overlay -->
                                            <div style="position: absolute; inset: 0; background: linear-gradient(135deg, rgba(30, 58, 138, 0.2), rgba(59, 130, 246, 0.2)); pointer-events: none;"></div>
                                        </div>
                                    @else
                                        <div style="aspect-ratio: 4/3; background: rgba(255,255,255,0.1); backdrop-filter: blur(20px); border-radius: 20px; display: flex; align-items: center; justify-content: center; border: 2px solid rgba(255,255,255,0.2); box-shadow: 0 20px 60px rgba(0,0,0,0.3);">
                                            <i class="fas fa-image" style="font-size: 100px; color: rgba(255,255,255,0.3);"></i>
                                        </div>
                                    @endif
                                </div>
                            </div>
