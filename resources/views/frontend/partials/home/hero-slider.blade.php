@php
    ...
    try {
        ...
        $heroColors = [
            ...
            'secondary' => App\Models\Setting::where('key', 'hero_secondary_color')->value('value') ?? '#f97316',
        ...
        $heroSecondaryButton = [
            'text' => App\Models\Setting::where('key', 'hero_secondary_button_text')->value('value') ?? 'Pelajari Lebih Lanjut',
            'link' => App\Models\Setting::where('key', 'hero_secondary_button_link')->value('value') ?? '/page/about',
            'show' => App\Models\Setting::where('key', 'hero_secondary_button_show')->value('value') ?? '1',
        ];
    } catch (\Exception $e) {
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 80px; align-items: center; {{ $slider->image_position === 'left' ? 'direction: rtl;' : '' }}">
                                ...
                                <div style="color: #fff; {{ $slider->image_position === 'left' ? 'direction: ltr;' : '' }}" data-aos="fade-{{ $slider->image_position === 'left' ? 'left' : 'right' }}" data-aos-duration="1000">
                                    ...
                                    <h1 style="font-size: 48px; font-weight: 800; margin: 0 0 25px 0; line-height: 1.1; text-shadow: 0 2px 10px rgba(0,0,0,0.2);">
                                        ...
                                        @if($restWords)
                                            {{ $restWords }}
                                            <span style="background: linear-gradient(135deg, {{ $heroColors['accent'] }}, {{ $heroColors['secondary'] }}); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">{{ $lastWord }}</span>
                                        @else
                                            {{ $slider->title }}
                                        @endif
                                    </h1>
                                    ...
                                    <div style="position: relative; width: 100%; height: 100%;">
                                        ...
                                        @else
                                            <div style="position: relative; width: 100%; height: 100%; background: rgba(255,255,255,0.1); backdrop-filter: blur(20px); border-radius: 24px; display: flex; align-items: center; justify-content: center; border: 2px solid rgba(255,255,255,0.2); box-shadow: 0 25px 70px rgba(0,0,0,0.5); z-index: 1;">
                                                <i class="{{ $icons['image_fallback'] }}" style="font-size: 120px; color: rgba(255,255,255,0.3);"></i>
                                            </div>
                                        @endif
